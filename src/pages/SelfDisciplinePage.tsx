...
const SelfDisciplinePage: React.FC<SelfDisciplinePageProps> = ({ isLoggedIn, setIsLoggedIn }) => {
  ...
  const [smartRunState, setSmartRunState] = useState<'recommend' | 'running' | 'syncing' | 'done'>('recommend');
  const [recommendedRoute, setRecommendedRoute] = useState<{ name: string; distance: number } | null>(null);
